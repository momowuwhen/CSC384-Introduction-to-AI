...
def solve_planes(planes_problem, algo, allsolns,
    ...
    def planesCSP(planes_problem):
        ...
        flights_can_fly = planes_problem.can_fly
        flights_can_start = planes_problem.can_start


        var_array = []
        # C1: Each plane is only assigned flights that it is capable of flying (e.g., small planes cannot fly trans-Atlantic flights).
        for i in range(len(planes)):
            var_array.append([])
            can_fly = flights_can_fly(planes[i])
            # C2: Each plane’s initial flight can only be a flight departing from that plane’s initial location.
            can_start = flights_can_start(planes[i])
        ...
        # C3. The sequence of flights flown by every plane must be feasible. 
        # That is, if F2 follows F1 in a plane’s sequence of flights then it must be that F2 can follow F1 
        # (normally this would mean that F1 arrives at the same location that F2 departs). 
        all_feasible_sequence = []
        for sequence in can_follow:
            all_feasible_sequence.append(list(sequence))
        for flight in flights:
            all_feasible_sequence.append([flight, 'no flight'])
        all_feasible_sequence.append(['no flight', 'no flight'])
        ...
        for i in range(len(planes)):
            ...
            for j in range(len(can_fly) - 1):
                F1 = var_array[i][j]
                F2 = var_array[i][j + 1]
                constraint_list.append(TableConstraint("C3({}{},{}{})".format(i,j,i,j+1), [F1,F2], all_feasible_sequence))

        # C4. Certain flights terminate at a maintenance location. All planes must be serviced with a certain minimum frequency. 
        # If this minimum frequency is K then in the sequence of flights assigned to a plane at least one 
        # out every subsequence of K flights must be a flight terminating at a maintenance location. 
        # Note that if a plane is only assigned J flights with J < K, then it satisfies this constraint.
        required_values = maintenance_flights + ['no flight']
        for i in range(len(var_array)):
            for j in range(len(var_array[i]) - min_maintenance_frequency + 1):
                scope = var_array[i][j:j+min_maintenance_frequency]
                constraint_list.append(NValuesConstraint("C4({})".format(i), scope, required_values, 1, min_maintenance_frequency))
       
        # C5: Each flight must be scheduled (be part of some plane’s sequence of flights). And no flight can be scheduled more than once.
        vars = [var for row in var_array for var in row]
        for flight in flights:
            constraint_list.append(NValuesConstraint("C5", vars, [flight], 1, 1))
    
        return CSP("plane_schedule", vars, constraint_list)

    csp = planesCSP(planes_problem) #set to to your CSP 
    solutions, num_nodes = bt_search(algo, csp, variableHeuristic, allsolns, trace)
    ...
    for solu in solutions:
        ...
        for i in range(len(planes_problem.planes)):
            one_solution.append([planes_problem.planes[i]])
        for (var, val) in solu:
            if val != 'no flight':
                one_solution[int(var.name()[5])].append(val)
        all_solution.append(one_solution)
