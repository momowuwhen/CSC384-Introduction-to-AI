...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for posx in qi._dom:
            for posy in qj._dom:
                # not in the same col and not in the diagonal line
                if posx != posy and abs(i-j) != abs(posx-posy):
                    satisfyingAssignments.append([posx, posy])
                    satisfyingAssignments.append([posy, posx])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], all_cons, partial_cons)
        return x
