...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty(): # no more unassigned variable
        if trace: print("{} Solution Found".format(csp.name()))
        solu = []
        for var in csp.variables():
            solu.append((var, var.getValue()))
        return [solu]
    ...
    for val in var.curDomain(): # iterate on current domain
        if trace: print("==> {} = {}".format(var.name(), val))
        var.setValue(val)
        noDWO = True
        for cnstr in csp.constraintsOf(var):
            if cnstr.numUnassigned() == 1:
                if FCCheck(cnstr, var, val) == "DWO": # prune future variables
                    noDWO = False
                    if trace: print("<==falsified constraint DWO\n")
                    break
        if noDWO:
            curr_solu = FC(unAssignedVars, csp, allSolutions, trace)
            all_solu.extend(curr_solu)
            # if only need one solution and one solution is already found
            if not allSolutions and len(all_solu):
                Variable.restoreValues(var, val)
                break
        
        Variable.restoreValues(var, val) # restore values pruned by this assignment
    var.unAssign() # undo addignment to var
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    #util.raiseNotDefined()
    if unAssignedVars.empty(): # no more unaddigned variables
        if trace: print("{} Solution Found".format(csp.name()))
        solu = []
        for var in csp.variables():
            solu.append((var, var.getValue()))
    ...
    for val in var.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(var), csp, var, val) == "DWO":
            ...
            noDWO = False
        if noDWO:
            curr_solu = GAC(unAssignedVars, csp, allSolutions, trace)
            if curr_solu:
                all_solu.extend(curr_solu)
            if not allSolutions and len(all_solu): # if only need to find one solution and already found
                var.restoreValues(var, val)
                break
        var.restoreValues(var, val) # restore values prunned by this assignment
    var.unAssign() # undo assignment to var
